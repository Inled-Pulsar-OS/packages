#!/usr/bin/env python3
"""Discord Gateway · UI layer — cross-renderer xui host + HTTP panel.
...
``~/.config/sayri/secrets.json`` (token), ``authorizations_<instance>.json``
(guests) and ``pairing_pin_<instance>.json``.
"""
...
try:
    from gateway import AUTH_FILE, DEFAULT_CONFIG_DIR, SHARED_PIN_FILE, INSTANCE_ID  # type: ignore
    ...
    AUTH_FILE = DEFAULT_CONFIG_DIR / f"authorizations_{INSTANCE_ID}.json"
...
PANEL_TITLE = "Discord Gateway · Sayri"
...
def delete_secret(key: str = TOKEN_KEY) -> bool:
    try:
        data = json.loads(SECRETS_FILE.read_text(encoding="utf-8")) if SECRETS_FILE.is_file() else {}
    except Exception:  # noqa: BLE001
        return False
...
def get_guests() -> bool:
    try:
        ...
        return bool(data.get("allow_channel_guests", False))
    except Exception:  # noqa: BLE001
        return False


def set_guests(enabled: bool) -> None:
    try:
        data = json.loads(AUTH_FILE.read_text(encoding="utf-8")) if AUTH_FILE.is_file() else {}
    except Exception:  # noqa: BLE001
        data = {}
    data["allow_channel_guests"] = bool(enabled)
...
class DiscordWizard:
    ...
    def __init__(self) -> None:
        ...
        self.val: dict = {"token": get_secret(), "guests": get_guests()}
    ...
    def _home_screen(self) -> dict:
        ...
        lines = [
            ...
            f"channel guests: {'allowed' if st['guests'] else 'denied'}",
        ...
        return screen(
            "Discord Gateway · Sayri",
            [text("Discord bot connection", accent=True),
             text("\n".join(lines), dim=True),
             note("Pair your phone in chat with /pair <PIN>. The token is stored in secrets.json.", "info")],
    ...
    def _configure_screen(self) -> dict:
        return screen(
            ...
            [
                note("1. Discord Developer Portal → bot → copy the Token\n"
                     "2. Enable 'Message Content Intent' (Privileged Gateway Intents)\n"
                     "3. Invite the bot to your server and talk with /sayri", "info"),
                entry("token", "Bot Token", default=self.val.get("token", ""),
                      secret=True, hint="Paste the token and save."),
                check("guests", "Allow guests in authorized channels",
                      default=bool(self.val.get("guests", False))),
            ],
            footer=[button("back", "Back"), button("save", "Save", kind="primary")],
            id="configure", step="Step 1/2",
        )
    ...
    def _pin_screen(self) -> dict:
        ...
        return screen(
            ...
            [text(pin, accent=True),
             sub("Type it in Discord: /pair " + pin),
    ...
    def _save(self, value: dict) -> dict:
        ...
        if "guests" in value and bool(value["guests"]) != get_guests():
            set_guests(bool(value["guests"]))
            self.val["guests"] = bool(value["guests"])
        self.idx = 0
        return self._home_screen()


# ---------------------------------------------------------------- surfaces
def wizard_app() -> DiscordWizard:
    return DiscordWizard()


def run_tui(stream: Any = None, out: Any = None) -> int:
    """Terminal surface — interactive TUI or piped line mode. No CLI parsing."""
    return run_cli(DiscordWizard(), stream=stream, out=out)
...
def panel_page(wizard: Optional[DiscordWizard] = None, transport: Optional[dict] = None) -> str:
...
# ------------------------------------------------------------ HTTP panel
def make_handler(wizard: DiscordWizard, status_fn: Optional[Callable[[], Any]] = None,
...
def serve_http(host: str = "127.0.0.1", port: int = 8756,
               wizard: Optional[DiscordWizard] = None) -> None:
    ...
    print(f"Discord panel →  http://{host}:{port}")
